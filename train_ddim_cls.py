...
parser.add_argument('--mel-length', type=int, default=400)
parser.add_argument('--val-num', type=int, default=1)
...
@torch.no_grad()
def eval_ddim(autoencoder, unet, scheduler, eval_loader, epoch=0, global_step=0, ddim_steps=100, eta=0.8):
    # noise generator for eval
    generator = torch.Generator(device=accelerator.device).manual_seed(args.random_seed)
    scheduler.set_timesteps(ddim_steps)

    unet.eval()
    for step, (mixture, _, target, _, _, cls, _, event_tensor, file_id) in enumerate(eval_loader):
        # compress by vae
        # mixture = autoencoder.mel2emb(logmel(mixture))*args.scale_factor
        mixture = minmax_norm_diff(logmel_val(mixture)).unsqueeze(1)
        # timbre = logmel(timbre)
        cls = cls.long()
        # target = autoencoder.mel2emb(logmel(target))*args.scale_factor
        target = minmax_norm_diff(logmel_val(target)).unsqueeze(1)
        ...
        for t in scheduler.timesteps:
            ...
            model_output = unet(x=pred, t=t, mixture=mixture, cls=cls,
                                timbre=None, timbre_feature=None, event=event_tensor)
        ...
        wav = autoencoder.mel2wav(pred.squeeze(1))

        save_plot(pred, f'{args.log_dir}/pic/{epoch}_pred_{file_id[0]}.png')
        save_audio(f'{args.log_dir}/audio/{epoch}_pred_{file_id[0]}', 16000, wav)

        if os.path.exists(f'{args.log_dir}/pic/{file_id[0]}.png') is False:
            # target = autoencoder.emb2mel(target)
            target = reverse_minmax_norm_diff(target)
            target_wav = autoencoder.mel2wav(target.squeeze(1))
            save_plot(target, f'{args.log_dir}/pic/{file_id[0]}.png')
            save_audio(f'{args.log_dir}/audio/{file_id[0]}', 16000, target_wav)

            mixture = reverse_minmax_norm_diff(mixture)
            mixture_wav = autoencoder.mel2wav(mixture.squeeze(1))
            save_plot(mixture, f'{args.log_dir}/pic/mixture_{file_id[0]}.png')
            save_audio(f'{args.log_dir}/audio/mixture_{file_id[0]}', 16000, mixture_wav)
...
if __name__ == '__main__':
    ...
    val_set = TSEDataset(data_dir=args.data_path, subset='val', length=10,
                         use_timbre_feature=args.use_timbre_feature, timbre_path=args.timbre_path,
                         mel_length=1000)
    test_set = TSEDataset(data_dir=args.data_path, subset='test', length=10,
                          use_timbre_feature=args.use_timbre_feature, timbre_path=args.timbre_path,
                          mel_length=1000)
    ...
    logmel = LogMelSpectrogram(mel_length=args.mel_length).to(accelerator.device)
    ...
    for epoch in range(args.epochs):
        ...
        for step, batch in enumerate(tqdm(train_loader)):
            # compress by vae
            mixture, _, target, _, _, cls, _, event_tensor = batch
