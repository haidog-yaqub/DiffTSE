...
parser.add_argument('--timbre-path', type=str, default='data/fsd2018/timbre_mae')
parser.add_argument('--audio-length', type=int, default=10)
parser.add_argument('--use-timbre-feature', type=bool, default=True)
...
parser.add_argument('--diffusion-config', type=str, default='model/DiffTSE.yaml')
...
@torch.no_grad()
def eval_ddim(autoencoder, unet, scheduler, eval_loader, epoch=0, global_step=0, ddim_steps=50, eta=0):
    # noise generator for eval
    generator = torch.Generator(device=accelerator.device).manual_seed(args.random_seed)
    scheduler.set_timesteps(ddim_steps)

    unet.eval()
    for step, (mixture, timbre, target, _, _, _, timbre_feature, file_id) in enumerate(eval_loader):
        # compress by vae
        mixture = autoencoder.module.mel2emb(logmel(mixture))
        timbre = logmel(timbre)
        target = autoencoder.module.mel2emb(logmel(target))
        ...
        for t in scheduler.timesteps:
            ...
            model_output = unet(x=pred, t=t, mixture=mixture, timbre=timbre, timbre_feature=timbre_feature)
            pred = scheduler.step(model_output=model_output, timestep=t, sample=pred,
                                  eta=eta, generator=generator).prev_sample

        pred = autoencoder.module.emb2mel(pred)
        wav = autoencoder.module.mel2wav(pred)
        ...
        if os.path.exists(f'{args.log_dir}/pic/{file_id[0]}.png') is False:
            target = autoencoder.module.emb2mel(target)
            target_wav = autoencoder.module.mel2wav(target)
...
if __name__ == '__main__':
    ...
    for epoch in range(args.epochs):
        ...
        for step, batch in enumerate(tqdm(train_loader)):
            # compress by vae
            mixture, timbre, target, _, _, _, timbre_feature = batch
            ...
            timesteps = torch.randint(0, noise_scheduler.num_train_timesteps, (args.batch_size,),
                                      device=accelerator.device,).long()
            noisy_target = noise_scheduler.add_noise(target, noise, timesteps)

            # inference
            pred = unet(x=noisy_target, t=timesteps, mixture=mixture, timbre=timbre, timbre_feature=timbre_feature)
            ...
            if global_step % args.log_step == 0:
                n = open(args.log_dir + 'ddim_log.txt', mode='a')
